...
def load_cosmas_npz_by_path(path):
    """
...
# ============================================================
# Onglet 1 : viewer classique (reprend ton code au max)
# ============================================================
...
class CosmasResultsTab(ttk.Frame):
    ...
    def refresh_index(self, init=False):
        ...
        if not self.data_index:
            self._set_info(f"Aucun fichier .npz trouvé dans {self.folder}\n"
                           f"Tu peux cliquer 'Run Simulation' pour en générer.")
            self.combo_M["values"] = []
            self.combo_h["values"] = []
            self.combo_k["values"] = []
    ...
    def update_file_options(self):
        ...
        if not files:
            self.var_file.set("")
            self._set_info("Aucun run pour ces paramètres.")
            return
    ...
    def preview_selected(self):
        ...
        try:
            d = load_cosmas_npz_by_path(path)
        except Exception as e:
            self._set_info(f"Erreur de lecture: {e}")
            return
    ...
    def show_selected(self):
        fname = self.var_file.get()
        if not fname:
            messagebox.showinfo("Info", "Choisis un fichier à afficher.")
            return
        path = os.path.join(self.folder, fname)
        ...
        try:
            ...
            messagebox.showerror(
                "Import error",
                f"Impossible d'importer display_order_vs_tsp depuis cosmas13.py\n{e}"
            )
            return
    ...
    def open_run_dialog(self):
        """
        Popup pour lancer une simulation cosmas via cosmas13.py
        """
        popup = tk.Toplevel(self)
        ...
        hint = tk.Label(
            popup,
            text="Note: la simulation peut être longue si LKH est appelé.",
            fg="gray"
        )
        hint.pack(pady=10)
        ...
        def run_and_close():
            ...
            popup.destroy()
            self.refresh_index()
            messagebox.showinfo("OK", "Simulation terminée et sauvegardée.")
