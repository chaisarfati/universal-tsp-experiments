...
if __name__ == "__main__":
    ...
    # =========================================================
    # ===== Étape 3: paramètres dyadiques =====
    # =========================================================
    # Scale dyadique (2^(2t) carrés)
    t = 5

    # ---------------------------------------------------------
    # Paramètres géométriques UNITAIRES (échelle [0,1]^2)
    # IMPORTANT:
    # Ces paramètres NE dépendent PAS de t.
    # Le scaling par la taille du carré (2^{-t}) est fait
    # exclusivement dans collect_backtracks_by_square_kravaris.
    # ---------------------------------------------------------
    l0 = 0.30   # longueur unitaire du backtrack
    w0 = 0.08   # largeur unitaire du strip

    # Budget de recherche (oracle heuristique)
    angle_steps = 180
    max_tries = 80

    # =========================================================
    # ===== Étape 4: collecte des backtracks (Kravaris) =====
    # =========================================================
    results = collect_backtracks_by_square_kravaris(
        points=points,
        order=order,
        t=t,
        l0=l0,
    ...
    print("\n=== SUMMARY ===")
    print(f"dyadic scale t={t}")
    print(f"total dyadic squares = {len(generate_dyadic_squares(t))}")
    print(f"squares with backtrack found = {len(results)}")

    # =========================================================
    # ===== Étape 5: ANALYSE QUANTITATIVE (paper-aligned) =====
    # =========================================================
    # Construction du set S :
    # Kravaris-style => UN SEUL point par carré (le pivot p).
    selected_points = [
        tuple(st["p"])
        for st in results.values()
    ]
    ...
    if n_final < 3:
        print("Not enough points for path comparison.")
    else:
        # -----------------------------------------------------
        # Ordre heuristique restreint à S
        # -----------------------------------------------------
        ...
        # -----------------------------------------------------
        # Coût TSP (fixé à 2 par convention / confort)
        # -----------------------------------------------------
        #tsp_cost = 2.0
        # --- TSP exact (LKH) ---
    ...
    # =========================================================
    # ===== Étape 6: visualisation et debug =====
    # =========================================================
    display_analysis_path(
    points=points,
    order=order,
    results=results,
    title=f"Kravaris analysis mode (t={t})"
    )
