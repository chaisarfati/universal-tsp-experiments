...
def visualize_state_in_square(points, state, l=0.3, w=0.08):
    sq = state["_square"]
    t = sq["t"]
    sq_idx = sq["sq_idx"]

    # On doit retrouver le carré dyadique
    squares = generate_dyadic_squares(t)
    (x0, y0), size = squares[sq_idx]

    square_order = restrict_order_to_square(
        list(range(len(points))),
        set(np.where(filter_points_in_square(points, x0, y0, size))[0])
    )
    ...
    # droite locale L0
    ts = np.linspace(-1, 1, 300)
    line = np.array([p + tt * d for tt in ts])
    ax.plot(line[:, 0], line[:, 1], "--k", label="L0")

    # points
    ax.scatter(ordered[:, 0], ordered[:, 1], s=20, color="black", alpha=0.4)
    ax.scatter([p[0]], [p[1]], s=80, color="red", label="pivot p")
...
if __name__ == "__main__":
    ...
    # Mapping pivot point -> backtrack state
    pivot_state_map = {}

    for st in results.values():
        p = tuple(st["p"])
        # on ne garde que les pivots qui sont dans S
        if any(np.allclose(p, s) for s in S):
            pivot_state_map[p] = st
